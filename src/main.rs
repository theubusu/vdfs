...
use clap::Parser;
use std::{collections::HashMap, fs::File, io::{self, Cursor, Read}, path::PathBuf};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let file_path = args.in_path;

    println!("Input file: {}", file_path);
    let mut file = File::open(file_path)?;
    let output_path = PathBuf::from(&args.out_path);
    ...
    if !debug && &vb.signature != b"VDFS" {
        return Err("invalid VDFS signature".into());
    }
    ...
    if debug{println!("{:?}", sb1)};

    if sb1.read_only != 1 {
        println!("Warning: This is not a read-only image and the extraction might be broken!");
    }

    //superblock 2 @ 0x400
    let sb2: Vdfs4SuperBlock = file.read_le()?;
    if debug{println!("{:?}", sb2)};

    if sb1 != sb2 {
        println!("Warning: Two superblocks differ!");
    }
    ...
    loop {
        ...
        if magic == b"fsmb" {
            if verbose{println!("BTREES BLOCK {}, Offset: {} - FSMB bitmap (fmsb)", btables_block, pos)};
            file.seek(SeekFrom::Current((1 * block_size as i64) - 4))?; //1 block

        } else if magic == b"inob" {
            if verbose{println!("BTREES BLOCK {}, Offset: {} - Inode bitmap (inob)", btables_block, pos)};
            file.seek(SeekFrom::Current((1 * block_size as i64) - 4))?; // 1 block

        } else if magic == b"eHND" { // btree header
            file.seek(SeekFrom::Current(-4))?;                
            let btree_head: Vdfs4RawBtreeHead = file.read_le()?;
            if debug{println!("{:?}", btree_head)};
            if verbose{
            ...
            if btree_n == 1 { //catalog tree - 1 i know
                for i in 0..node_descr.recs_count {
                    ...
                    if verbose{println!("-- KEY {} - Object ID: {}, Parent ID: {}, Type: {}({:?}), Name: {}", i, key.object_id, key.parent_id, key.record_type, key_type, key.name_str())};
                    ...
                    if node_descr.node_type == 1 { //special type for root inode?
                        ...
                        if debug{println!("{:?}", folder_record)};
                        my_inodes.insert(key.object_id, MyInode { _parent_id: key.parent_id, kind: key_type.clone(), _name: key.name_str(), meta: InodeMeta::Folder(folder_record)});

                    } else if key_type == KeyRecordType::VDFS4_CATALOG_FILE_RECORD {
                        let file_record: Vdfs4CatalogFileRecord = file.read_le()?;
                        if debug{println!("{:?}", file_record)};                        
                        my_inodes.insert(key.object_id, MyInode { _parent_id: key.parent_id, kind: key_type.clone(), _name: key.name_str(), meta: InodeMeta::File(file_record)});

                    } else if key_type == KeyRecordType::VDFS4_CATALOG_HLINK_RECORD {
                        let hlink_record: Vdfs4CatalogHlinkRecord = file.read_le()?;
                        if debug{println!("{:?}", hlink_record)};
                    }
            ...
            if verbose{println!("- Found nothing! Skip 1 block")};
            file.seek(SeekFrom::Current((1 * block_size as i64) - 4))?; // 1 block
        }
    }

    //making TAR
    let out_tarfile = File::create(&output_path).unwrap();
